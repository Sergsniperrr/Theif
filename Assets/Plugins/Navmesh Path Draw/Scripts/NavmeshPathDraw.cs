...
public class NavmeshPathDraw : MonoBehaviour
{
    ...
    public void Draw()
    {
        ...
        if (Physics.Raycast(destination, -Vector3.up, out downHit, Mathf.Infinity, groundLayers)) {
            validatedDesPos = new Vector3(destination.x, downHit.transform.position.y, destination.z);
        }else{
            validatedDesPos = destination;
        }
